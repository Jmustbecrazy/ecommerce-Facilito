...
class Link < ActiveRecord::Base
  ...
  def create_attachment_links
    product.attachments.each do |attachment|
      #creamos un link con cada attachement
  ...
  def set_defaults
  	...
  	#en caso especificamos ignora el 0
  	self.downloads ||= 0
  end

  def send_email
  	#clase creada en la carpeta mailer(deliver_now => metodo de ruby en nuevas versiones)
